...
def count_gt0_transform(thresh=0.0):
...
async def main():
    ...
    parser.add_argument(
        "--fc1-pattern",
        type=str,
        help="The pattern to match the layer name for the FC1 layer.",
    )
    ...
    writer = await get_tensor_writer(
        model,
        output_shape=(num_layers, ffn_size),
